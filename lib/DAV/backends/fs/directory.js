    ...
    getChildren: function(cbfsgetchildren) {
        ...
        Async.readdir(this.path)
             ...
             .each(function(file, cbnextdirch) {
                 // fixing Async.readdir bug which returns full path instead of just last part with windows delimiters
                 file.path = file.path.substring(file.path.lastIndexOf(Path.sep)+1);
